...
export function Cocina() {
  ...
  // Filtrar productos de cocina
  const productosCocina = local.almacen.filter(p => p.categoria === 'cocina');
  const productosCocinaAlmacenCentral = datos.almacenCentral.filter(p => p.categoria === 'cocina');
  ...
  const entradas = datos.movimientos.filter(m => {
    const productoEsCocina = productosCocina.some(p => p.id === m.productoId);
    return (
      m.tipo === 'suministro' &&
      m.destino === 'local-001' &&
      productoEsCocina &&
  ...
  const productosBajoStock = productosCocina.filter(p => p.cantidad < 10);

  // Preparar datos para el gráfico de movimientos por producto
  const datosGrafico = productosCocina.map(producto => {
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="text-gray-900">Cocina - Cafe Avellaneda</h1>
            <p className="text-gray-600">Reportes e inventario de cocina</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden mb-8">
        <div className="p-6 bg-orange-50 border-b border-orange-200">
          <h2 className="text-gray-900">Inventario de Cocina - Local</h2>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {productosCocina.map(producto => {
                const productoCentral = productosCocinaAlmacenCentral.find(p => p.id === producto.id);
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-orange-700 mb-4">
                Los siguientes productos de cocina tienen menos de 10 unidades. Se recomienda solicitar suministro del almacén central.
              </p>
